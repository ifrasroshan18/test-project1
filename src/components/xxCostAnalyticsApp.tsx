...
export default function CostAnalyticsApp() {
  ...
  // Legend payload for pie (values + percentages)
  const pieLegendPayload = useMemo(() => {
    const total = pieData.reduce((s, p) => s + (Number(p.value) || 0), 0) || 1;
        ...
        <div className="p-4 bg-white rounded shadow space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              ...
              <div className="mt-1 max-h-40 overflow-auto border rounded p-2 grid grid-cols-1 gap-1">
                {headerRow.map((h, idx) => (
          ...
          <div>
            ...
            />
            <div className="mt-2 max-h-48 overflow-auto border rounded p-2 grid grid-cols-1 gap-1">
              {headerRow
                .map((h, idx) => ({ h, idx }))
            ...
            <div className="mt-3 grid grid-cols-1 gap-4">
              ...
              <div className="flex items-end">
                <button className="px-3 py-2 bg-white text-black border rounded" onClick={() => setSelectedDims([])}>
                  Clear Dimensions
                </button>
              </div>
            </div>
          ...
          <div>
            ...
            <button className="mt-2 px-3 py-1 bg-white text-black border rounded" onClick={addFilterRule}>
              + Add Rule
            </button>
            <button className="mt-2 ml-2 px-3 py-1 bg-white text-black border rounded" onClick={clearFilterRules}>
              Clear Rules
            </button>
          ...
          {primaryMetricIdx == null && (
              <div className="p-3 bg-yellow-100 text-black rounded text-sm">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-8">
            <div className="bg-white rounded p-4 shadow" ref={lineRef as any}>
              ...
              <div className="mt-2 flex space-x-2">
                <button onClick={() => downloadCSV(lineSeriesData.rowsWide ?? [], "trend.csv")} className="px-3 py-1 bg-white text-black border rounded">
                  Export CSV
                </button>
                <button onClick={() => downloadPNG(lineRef, "trend.png")} className="px-3 py-1 bg-white text-black border rounded">
                  Export PNG
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded p-4 shadow" ref={pieRef as any}>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={pieData} dataKey="value" nameKey="group" outerRadius={110} labelLine={false} paddingAngle={2}>
                    {pieData.map((_, i) => (
                      <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(v: any) => (typeof v === "number" ? v.toLocaleString(undefined, { style: "currency", currency: "USD" }) : v)} />
                  <Legend {...({ payload: pieLegendPayload } as any)} layout="vertical" align="right" verticalAlign="middle" wrapperStyle={{ fontSize: 12, color: "#000" }} />
                </PieChart>
              </ResponsiveContainer>
              <div className="mt-2 flex space-x-2">
                <button onClick={() => downloadCSV(pieData, "share-by-dimensions.csv")} className="px-3 py-1 bg-white text-black border rounded">
                  Export CSV
                </button>
                <button onClick={() => downloadPNG(pieRef, "share.png")} className="px-3 py-1 bg-white text-black border rounded">
                  Export PNG
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-8">
            <div className="bg-white rounded p-4 shadow" ref={barRef as any}>
              ...
              <div className="mt-2 flex space-x-2">
                <button onClick={() => downloadCSV(barData, "totals.csv")} className="px-3 py-1 bg-white text-black border rounded">
                  Export CSV
                </button>
                <button onClick={() => downloadPNG(barRef, "totals.png")} className="px-3 py-1 bg-white text-black border rounded">
                  Export PNG
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded p-4 shadow" ref={areaRef as any}>
              ...
              <div className="mt-2 flex space-x-2">
                <button onClick={() => downloadCSV(areaData, "cumulative.csv")} className="px-3 py-1 bg-white text-black border rounded">
                  Export CSV
                </button>
                <button onClick={() => downloadPNG(areaRef, "cumulative.png")} className="px-3 py-1 bg-white text-black border rounded">
                  Export PNG
                </button>
              </div>
            </div>
